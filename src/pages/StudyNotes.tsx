  ...
  return (
    <MobileLayout>
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-4 flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Study Notes</h1>
            ...
            <Link key={subject.id} to={`/study-notes/${subject.id}`}>
              <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-200 cursor-pointer group h-full">
                ...
                <CardHeader className="pb-3">
                  <CardTitle className="text-base sm:text-lg group-hover:text-[#009dff] transition-colors">
                    {subject.title}
                  </CardTitle>
                  <p className="text-xs sm:text-sm text-gray-600 line-clamp-2">{subject.description}</p>
                </CardHeader>
